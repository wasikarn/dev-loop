...
argument-hint: "[pr-number] [jira-key?] [--quick?] [--full?] [--focused area?] [Author|Reviewer?]"
...
**Args:** `$0`=PR# (required) · `$1`=Jira key or Author/Reviewer · `$2`=Author/Reviewer · `--quick`=fast SDK-only · `--full`=force 3-reviewer debate · `--focused [area]`=specialist only. Flags (`--quick`/`--full`/`--focused`) are detected by pattern matching — position-independent.
**Modes:** Author = fix code · Reviewer = comment only (in Thai) · --quick = fast SDK path, no debate · --focused [area] = specialist only (errors/types/tests/api/migrations)
